...
class ListBuilder extends Controller{
    ...
    private $_table_data_list_status = 'status';  //表格数据列表状态字段名
    ...
    public function addTopButton($type, $attribute = null){
        switch($type){
            ...
            case 'resume': //添加启用按钮(禁用的反操作)
                ...
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'resume', 'field'=>$this->_table_data_list_status, 'model' => $my_attribute['model']));
            ...
            case 'forbid': //添加禁用按钮(启用的反操作)
                ...
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'forbid', 'field'=>$this->_table_data_list_status, 'model' => $my_attribute['model']));
            ...
            case 'recycle': //添加回收按钮(还原的反操作)
                ...
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'recycle', 'field'=>$this->_table_data_list_status, 'model' => $my_attribute['model']));
            ...
            case 'restore': //添加还原按钮(回收的反操作)
                ...
                $my_attribute['class'] = 'btn btn-success ajax-post confirm';
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'restore', 'field'=>$this->_table_data_list_status, 'model' => $my_attribute['model']));
            ...
            case 'delete': //添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                ...
                $my_attribute['class'] = 'btn btn-danger ajax-post confirm';
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'delete', 'field'=>$this->_table_data_list_status, 'model' => $my_attribute['model']));
     ...
     * @author jry <598821125@qq.com>
     */
    public function setTableDataListStatus($table_data_list_status){
        $this->_table_data_list_status = $table_data_list_status;
        return $this;
    }
    ...
    public function addRightButton($type, $attribute = null){
        switch($type){
            ...
            case 'forbid': //改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                ...
                $my_attribute['0']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'resume', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
                $my_attribute['1']['title'] = '禁用';
                $my_attribute['1']['class'] = 'label label-warning ajax-get confirm';
                $my_attribute['1']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'forbid', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
            ...
            case 'hide': //改变记录状态按钮，会更具数据当前的状态自动选择应该显示隐藏/显示
                ...
                $my_attribute['2']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'show', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
                $my_attribute['1']['title'] = '隐藏';
                $my_attribute['1']['class'] = 'label label-info ajax-get confirm';
                $my_attribute['1']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'hide', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
            ...
            case 'recycle':
                ...
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'recycle', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
            ...
            case 'restore':
                ...
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'restore', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
            ...
            case 'delete':
                ...
                $my_attribute['class'] = 'label label-danger ajax-get confirm';
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus', array('status' => 'delete', 'field'=>$this->_table_data_list_status, 'ids' => '__data_id__', 'model' => $my_attribute['model']));
            ...
            if($this->_right_button_list){
                foreach($this->_right_button_list as $right_button){
                    ...
                    if($right_button['type'] === 'forbid' || $right_button['type'] === 'hide'){
                        $right_button = $right_button[$data[$this->_table_data_list_status]];
                    }
