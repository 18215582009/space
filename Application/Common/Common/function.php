 ...
 * @author jry <598821125@qq.com>
 */
function member_md5($str, $auth_key){
    if(!$auth_key){
        $auth_key = C('AUTH_KEY');
    }
 ...
 * @author jry <598821125@qq.com>
 */
function get_member_info($id, $field){
...
function get_cover($id, $type){
    $url = S('UPLOAD_REAL_PATH_'.$id);
    if(!$url && $id){
        $map = array();
        $map['status'] = array('eq', 1);
        $upload_info = D('PublicUpload')->where($map)->find($id);
        $url = $upload_info['real_path'];
        S('UPLOAD_REAL_PATH_'.$id, $url, 3600);
    }
    if(!$url){
        switch($type){
            case 'default' : //默认图片
                $url = C('TMPL_PARSE_STRING.__IMG__').'/logo/default.gif';
                break;
            case 'avatar' : //用户头像
                $url = C('TMPL_PARSE_STRING.__IMG__').'/avatar/avatar'.rand(1,7).'.png';
                break;
            default: //文档列表默认图片
                $url = '';
                break;
