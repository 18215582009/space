...
class MemberMessageModel extends Model{
    ...
    public function addMessageContent($from_uid, $title, $content, $url, $url_args, $type){
        ...
        $message_content_id = D('MemberMessageContent')->add($data_content);
    ...
    public function getHaventReadMessage($to_uid){
        ...
        $messages = $this->where($where)->order('message_id desc')->limit(99999)->select();
    ...
    public function getContent($message_content_id){
        ...
        if(empty($content)){
            $content = D('MemberMessageContent')->find($message_content_id);
            if($content){
