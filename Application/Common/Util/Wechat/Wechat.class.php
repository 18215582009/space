...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @param string $url
	 */
	private function http_get_all($url){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, FALSE);
		...
		curl_close($oCurl);
		if(intval($aStatus["http_code"])==200){
		    return array_merge(array('mediaBody' => $sContent), $aStatus);
		}else{
			return false;
		}
	}
	...
	public function getMedia($media_id,$is_video=false){
		...
		if ($result)
		{
            ...
            if (is_string($mediaBody)) {
                $json = json_decode($mediaBody,true);
	 ...
	 * 2016-05-19
	 */
	public function getMediaAll($media_id,$is_video=false){
		if (!$this->access_token && !$this->checkAuth()) return false;
		//原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
		//如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
		$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
		$result = $this->http_get_all($url_prefix.self::MEDIA_GET_URL.'access_token='.$this->access_token.'&media_id='.$media_id);
		if ($result)
		{
		    $mediaBody = $result['mediaBody'];
            if (is_string($mediaBody)) {
