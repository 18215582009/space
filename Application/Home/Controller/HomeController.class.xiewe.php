        ...
        if(!is_login()){//是否登录
            ...
            if(!empty($is_exists) and false!==$is_exists){//如果存在自动登录
                 ...
                 if($data){
                    ...
                    // header('Location:'.session('__forward__').'');die;
                    // $this->redirect(session('__forward__'));
                 }
            }else{//自动注册
                ...
                if(!$memberWechat || false==$memberWechat){
                 ...
                 if($data){
                    // $requesturi = array();
                    // $requesturi[0] = 'http://126wenren.ejar.com.cn/Index/person.html';
                    // $requesturi[1] = 'http://126wenren.ejar.com.cn/Index/shareList.html';
                    // $requesturi[2] = 'http://126wenren.ejar.com.cn/Index/126.html';
                    // $requesturi[3] = 'http://126wenren.ejar.com.cn/Index/goodslist.html?type=70';
                    // $uri =  'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
                    // if (in_array($uri, $requesturi)) {
                    //   $this->redirect($uri);
                    // }else{
