...
/*-----------------入口接口 包含检测用户抽奖次数 返回已抽奖的奖券-----------------------*/
		public function Index(){
			...
			if(!empty($this->shopid)){
				$res = M('gy_document')->where('id='.$this->shopid)->field('title')->select();
				if(empty($res)){
				$arrayName = array('status' =>201,'content'=>'没有此商户');
				echo json_encode($arrayName);exit;
				}
				}else{
				$arrayName = array('status' =>201,'content'=>'没有商户号');
				echo json_encode($arrayName);exit;
			}
...
/*----———————————————————— 微信二维码—————————————————————————————————*/
   ...
   //    $data = 'PHP加密解密算法';    // 被加密信息
	...
	public function search(){
		$name = I("title");
		$res = M('gy_document')->query("select gy.title as gtitle,gy.id as gid,ct.id as cid,ct.title as ctitle from
								 gy_document as gy  
								 join gy_category as ct on ct.id=gy.category_id
								 where gy.title like'%".$name."%'");
		$result = Array();
		foreach ($res as $key => $value) {
			$result[$key]['title'] = $value['gtitle'];
			$result[$key]['id'] = $value['gid'];
			$result[$key]['cid'] = $value['cid'];
			$result[$key]['ctitle'] = $value['ctitle'];
		}
		if(!empty($result)){
			echo json_encode(array('data'=>$result,'status'=>200));
		}else{
			echo json_encode(array('data'=>'nodata','status'=>201));
		}
		exit;
	}
}
