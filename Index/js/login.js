...
function uploadImg(obj){
	...
	function setImg(e){
			var src=e.data.url;
			$(".add-img").before('<li class="dc-add-img" style="background:url('+src+');background-repeat: no-repeat;background-position: center;background-size:100%;background-size: cover;"><span class="dc-del" onclick="delimg(this)">×</span><input type="hidden" name="imgs" class="inpval" value="'+ src +'"></li>')
		var imgw = $(".dc-add-img").css("width");
		$(".dc-add-img").css("height", imgw);
		}
	//压缩图片
...
function  toSweiper(n,i){
	...
	addHtml+="<div class='swiper-container'>"
		addHtml+="<div class='swiper-wrapper'>"
			...
			addHtml+="</div>"
			addHtml+="<div class='swiper_close'>×</div>"
			addHtml+="</div>"
